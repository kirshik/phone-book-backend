...
export class ContactResolver {
  ...
  async createContact(@Args('createContact') createContactInput: CreateContactInput): Promise<ContactEntity> {
  ...
  @Query(() => [ContactEntity])
  async searchContacts(@Args('searchQuery') searchQuery: string): Promise<ContactEntity[]> {
    return await this.contactService.searchContacts(searchQuery);
  }
}
